"""Telegram 通知 & 承認モジュール（hiphop）。rugbypicksの pipeline/telegram_notify.py を移植。

x-trend-bot の `TELEGRAM_TOKEN` / `CHAT_ID` を流用する（agent/.env の TELEGRAM_BOT_TOKEN /
TELEGRAM_CHAT_ID とは別物＝既存の記事生成bot @waxthink_bot とは独立した通知経路）。
値は環境変数を優先し、無ければ `~/x-trend-bot/.env` から読む（トークンはコードに書かない）。

用途:
  send    : 任意メッセージ送信（HTML、URL添付可）。
  approve : 承認/却下ボタン付きメッセージを送り、Telegram 上の回答を待って結果を返す。
  ask     : 複数選択肢ボタンを送り、選ばれたラベルを返す。

CLI:
  python3.11 agent/telegram_notify.py send --text "..."
  python3.11 agent/telegram_notify.py approve --text "この記事を公開して良い？" --timeout 300
    → 承認: exit 0 / 却下: exit 10 / タイムアウト: exit 20 / 設定なし: exit 30
  python3.11 agent/telegram_notify.py ask --text "どっちにする？" --option A --option B
...
def ask(question: str, options: list[str], *, timeout: int = 600, poll: int = 3) -> Optional[str]:
    ...
    戻り値: 選ばれたラベル / None（タイムアウト・未設定）。番号返信（"1"〜）でも回答可。
    """
    token, chat = load_credentials()
    if not token or not chat:
...
def main(argv: Optional[list[str]] = None) -> int:
    ap = argparse.ArgumentParser(prog="agent/telegram_notify.py")
