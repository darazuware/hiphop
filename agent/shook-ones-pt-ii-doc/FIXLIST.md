...
- [x] C5. 波形に音声再生を追加 — `03-beat-making.html`。既存の`assets/waveform/sample-{jessica,kitty,dirtyfeet}-8s.wav`(8秒・既存アセット)を各波形revealの2秒前からdata-startして白波形(周辺音)→着色部分(実サンプル)へ連続再生。3サンプル説明後(cap-4, 22.9s)に`beat-sample-8s.wav`(合成ビート)を追加再生して「重ねた質感」を聴かせる。
  ...
  - **2026-09-01 七度目追記(交差の根本原因を特定・スポットライトのジオメトリを作り替え)**: 六度目の実装後も運営者から「Jessicaの上にKittyがきたり、入れ替わったりしている」と再指摘。前回セッションはメタラベルの`top:50%`が親行の高さアニメに追従する件を修正したが、**それは同時に存在した別のバグであって指摘の対象ではなかった**(この誤診の教訓は下記)。レンダー実物のMP4から`ffmpeg`でフレームを抜いて確認したところ真因が判明:
    - **原因1(本命・波形本体の交差)**: 六度目のスポットライト方式は3行すべてを共有スロット(実質top:0)へ`y`で寄せた上でアクティブ行の`height`を18→150へ伸ばす設計だった。波形は自分のボックス内で上下中央に描かれる(`inset:0` + `background-size: … 100%`)ため、**高さアニメの最中に波形の中心線がy=9からy=75まで移動し、待機中のKitty(y=35)・Dirty Feet(y=61)の線を物理的に通過**していた。金色のJessica波形がtealのKitty波形の上に滑り降り、退場時にまた通過する — これが指摘の正体。z-indexでは何も隠れない(どちらも発光しているため)。
      **対処**: アクティブ行を**下方向へ**伸ばし(height 104)、その下の行を**同じ86pxだけ押し下げる**方式へ変更。状態は idle / J tall / K tall / D tall の4つで、bottom(row_i)とtop(row_i+1)が遷移中も同一の`+86p`項を持つため**8pxの隙間が全フレームで保たれる**。交差は「起きにくい」ではなく**構造的に不可能**になった。ラベルも各行のスロットを`y`で追う(tall時+43・押し下げ時+86)ため、常時26px以上離れ、非アクティブを消して衝突を隠す必要がなくなった(全曲名が最後まで読める)。
    - **原因2(増幅要因・音と絵の窓ずれ)**: サンプル音源が自分のナレーションより1.5〜2秒早く開始していた(C5当初の「2秒前からdata-start」設計の名残。Kittyはナレーション12.9sに対し音は10.9s開始)。五度目の修正でrevealをaudioに完全一致させた結果、**前のサンプルが拡大表示中に次のサンプルの再生ヘッドが細い行を走る**状態になり、「入れ替わり」の印象を増幅していた。
      **対処**: wavを各ナレーション窓ちょうど(jessica 5.5s / kitty 4.5s / dirtyfeet 4.5s・末尾0.4sフェードアウト)に切り出した`sample-*-cut.wav`を作り、**そのトリム音源から波形PNG(`wave-*-cut.png` / `wave-*-cut-white.png`)を再生成**。1600px全幅=実際に鳴る全区間になったため`.h3-wave-highlight`は全行1600pxで済み、再生ヘッドが端から端まで走って音と同時に終わる(未着色の尾が消えた)。audioの`data-start`は各キャプション入場と同一(7.0 / 12.9 / 17.9)。
    - **付随して発覚した構造的欠陥**: 2行キャプション(cap-0・cap-2)の上端は`bottom:86` + padding28 + 54px/1.42×2行 + 28 = **y≈785**まで来るのに、波形スタックはy=861まで占有していた。**行の並べ方をどう変えても字幕と衝突し続けていたのはこれが理由**(二度目・四度目の追記で「解消した」としていたのは1行キャプション時だけの話)。ジャケットクラスタを`top:300px`→`208px`へ上げ(ズーム時の2行Jonesタグの下端が693→601へ)、波形スタックを`705px`→`616px`・156pxレーンとして確保(上15px・下13pxクリアランス)。ジャケットのズーム/リングもハンドオフ時刻(6.95 / 12.5 / 17.5 / 22.5)へ整列させ、ジャケット・波形・音が1つのビートで動くようにした。
    - **検証**: `npm run check` 0 error。`hyperframes snapshot`に加え、**レンダー実物のMP4から`ffmpeg`でフレームを抜いて**66.5 / 67.9(遷移中) / 68.1 / 68.6 / 73.5 / 76.0sの全状態を目視確認(順序保持・非重複・字幕クリアランス)。
    - **教訓(次回の自分へ)**: 四度目の追記が「snapshotは正しくレンダーだけが壊れていた」だったのに対し、今回は逆に**snapshotだけ見て「再現しない」と結論してしまい、一度誤った修正を出荷した**。運営者が見ているのは常にレンダー実物である以上、**この種の指摘は最初にレンダー済みMP4からフレームを抜いて確認する**こと。またレイアウト系の指摘は、行やラベルの配置を個別に調整する前に**キャプション最大行数を含めた縦方向の予算を先に計算する**(今回2行キャプションの785pxを見落としていたため同じ衝突を3回作り直していた)。
  - **2026-09-01 八度目追記(ハイハット矛盾・Kitty音源誤り・レイアウト全面刷新=ジャケット×波形3行)**: 運営者から2点指摘。①「3つの曲からと言いながら、最後にハイハット(ガスコンロ風のチッチッチッの音)入れてるから矛盾してる。ハイハットは入れなくていい」→ `beat-sample-8s.wav`をスペクトログラム解析した結果、高密度の広帯域バーストが規則的に並ぶ典型的なハイハットパターンで、**実際には完成後のShook Onesドラムループそのもの**だったと判明(「3枚を重ねた質感」の再現ではなく4つ目の非レコード音)。該当`<audio>`要素を完全削除。ナレーション本文(`part3-beat.mp3`・cap-4「この3つに、Havoc自身のハイハットを重ねて」)は事実として正しいため文言はそのまま維持し、それを裏切る音だけを鳴らさないようにした。
    ②「kitty→ストリングスなのにナレーション音声かハイハットになっている」→ `sample-kitty-8s.wav`をスペクトログラム解析すると前半(0-5.5s、旧`sample-kitty-cut.wav`が使っていた区間)は倍音構造のない広帯域ノイズ状で、弦楽器の特徴(ビブラートによる格子状の縞)が見えなかった。`assets/videos_source/tracklib-shook-ones-short.mp4`(Tracklib公式のUIアニメーション・Havocの喋りは無い71秒版)を2fpsでスキャンし色分けハイライトの正確な区間を実測: green=Dirty Feet 2.5-12s / blue=Kitty With the Bent Frame 12.5-24.5s / red=Jessica 25-58s。Kittyのハイライト窓内でより明確な倍音構造を持つ**t=18.3-22.8s**から音声を再抽出したところ、規則的な格子パターン(ビブラート)が確認できた。Dirty Feet(t=4.0-8.5s)・Jessica(t=27.0-32.5s)も同一動画・同一手法で再抽出し、スペクトルがそれぞれドラムの規則的パルス・ピアノの減衰型倍音と整合することを確認。`-ss`を`-i`の後に置くと(内部要因不明・要再現確認)フィルタ付き出力が無音(-91dB)になる現象に遭遇したため、**`-ss`は必ず`-i`の前に置く**運用に統一。
    ③(運営者指示・レイアウト刷新)「波形は曲全体分を写すとほぼ長くなってしまうので、曲全体→サンプリングする部分の前後まで拡大→サンプリングする部分だけ着色という風にしよう。そうすればジャケットを横並びから、(ジャケットの横に波形)×3行という構成にできる」→ ジャケット横並び3枚(上段)+別ブロックの波形スタック(下段)という旧構成(C1〜C5・七度目まで)を全廃し、**「ジャケット|タイトル+波形」を1行とする3行構成**へ全面作り替え。各行は`.h3-track-list`内で高さ130px・top:0/156/312pxに完全固定され、以後一切位置が動かない(七度目までの全バグは「行が共有スロットへ動く/伸びる」ことが根本原因だったため、動く要素自体を無くした)。「アクティブ」は各行の**自分の箱の中だけ**で完結するscale変化(ジャケット1→1.08・波形0.48/0.52→1.05/1.35・リング表示・opacity)で表現するため、他の行の座標と物理的に干渉しえない。
    波形の見た目は`tracklib-shook-ones-short.mp4`のSample Breakdown UI(暗い全体波形+実サンプル箇所だけ色付け)と同じ「読み」を狙ったが、**Tracklib社のブランドUIそのものは使わない**(STORYBOARD.mdの既定方針を継続)。商用の曲全体音源は入手できないため、実サンプル音源(`sample-*-cut.wav`)の前後に`anoisesrc`(pink noise + tremolo)で生成した"文脈"を約4秒ずつ結合し(`full-*.wav`)、そこから`full-*-white.png`(全体・白・低opacity)を生成。実サンプル区間だけは別途`real-*.png`(着色)としてレンダーし、結合波形内の正確な位置(offset=800px、200px/秒換算)に重ねて表示。ズームは`.h3-track-wave-wrap`の`transform-origin`を実サンプル区間の中心px(jessica 1350 / kitty・dirtyfeet 1250)に固定した**単一の`scale`tweenのみ**で実現(`left: calc(50% - center_x)`によりアンカー点が常にビューポート中央に来るため、位置移動を一切伴わない)。
    `npm run check`は0 error(info 3件のみ、既存許容パターン)。`hyperframes snapshot`とレンダー実物のフレーム抽出の両方で56〜87sの全ハンドオフを目視確認。関連ファイル: `wave-*.png`・`wave-*-cut*.png`・`beat-sample-8s.wav`は削除、`full-*-white.png`・`real-*.png`を新規追加。
  - **2026-09-01 九度目追記(波形の薄さ・ピッチ未反映・Kitty位置を再修正)**: 八度目リリース後も運営者から3点指摘「見た目は良くなったが、kittyの切り抜く位置が違う。それぞれピッチ落としてサンプリングしてるが、それも適用されていない。相変わらず波形が細いまんまでアップされていない」。
    - **波形の薄さ**: `convert -fuzz 5% -trim`で実測したところ、正規化前の`real-kitty.png`はキャンバス160pxのうち**34px(21%)**しか波形が描かれていなかった。`showwavespic`は音源の実振幅に比例して描く仕様のため、`sample-kitty-cut.wav`の音量が低いほど細く描かれていた。ピークをvolumedetectで実測し`-0.5dB`へブースト(kitty +7.7dB / jessica +9.7dB / dirtyfeet +1.5dB)してから波形PNGを再生成 → kitty 96px(60%)・jessica 108px(67%)・dirtyfeet 72px(45%)まで拡大。
    - **ピッチ未反映**: WebSearchで「Havoc Shook Ones sample pitched down slowed Jessica」を裏取りしたところ、the-breaks.com発の情報として**Havocは"Jessica"のピアノ5音を2種類の異なるピッチで落として2小節ループに再構成した**とあり、-14半音→+5半音という具体的数値も見つかったが、TheWrap等の主要ソースでは半音数の記載がなく裏取りしきれなかった。**このため半音数はナレーション本文には書かず、音声処理パラメータとしてのみ採用**(jessicaに-14半音、kitty/dirtyfeetは資料なしだが同系統として-7半音)。`asetrate`でピッチと速度を同時に落とす(実機サンプラー/ターンテーブルの挙動を再現)ため、先に短い元素材(jessica 2.45s→pitch後5.5s、kitty/dirtyfeet 3.0s→pitch後4.5s)を切り出してから処理し、最終尺は変更前と同じに揃えることでCSS/JS側(px幅・scale値)の変更を不要にした。
    - **Kittyの切り抜き位置**: 八度目のt=18.3-22.8sから**t=21.0-24.0s**へ変更。Kittyのハイライト窓(12.5-24.5s)内を0.5-1s刻みでスペクトログラム比較し、最も明瞭なビブラート格子パターンが出る区間を選定。**ただし音源はTracklib社の宣伝動画(実演奏の解説動画ではなくUIアニメーション)由来であり、「本当に単体ストリングスか」の確証は依然取れていない** — この不確実性は運営者にそのまま伝達済み。より確実な検証には`tracklib-breakdown.mp4`(458秒・Havoc本人が実演解説する長尺版)からの再抽出、または音源分離ツール(本機のPython 3.14aとtorchの組み合わせでは`demucs`がC拡張のsegfaultで動作せず断念)が必要。
    - **検証**: `npm run check` 0 error(HTML/CSS無変更、アセットのみ差し替え)。レンダー実物のフレーム抽出で波形の太さ改善を目視確認、スペクトログラムでピッチが低域へシフトしたことを確認。
  - **2026-09-01 十度目追記(ドラムのピッチ過剰・Kitty音源をTracklib外へ切替)**: 九度目リリース後も運営者から「字幕→ストリングス、ナレーション→ハイハット。実際はストリングスのはず。流している音がサンプリング部分じゃない。ドラムの音もピッチ音しすぎてまるで別物」と指摘。
    - **ドラムのピッチ過剰**: dirtyfeetへの-7半音ピッチダウン(九度目・半音数の裏付けなし)を完全撤回。トランジェント主体のドラムはピッチ楽器と違い大きくシフトすると別音に聞こえる、という基本を見落としていた。normalize(ピーク-0.5dB)のみ残し`asetrate`処理を削除、九度目以前の音そのものに復元。
    - **Kitty音源の根本的な出どころ間違い**: 八〜九度目まで4回、`tracklib-shook-ones-short.mp4`と`tracklib-breakdown.mp4`(Havocインタビュー内Kittyカード区間t=202-236s)の両方から複数タイムスタンプを試したが、**いずれもスペクトログラムでブロードバンドノイズ状(明瞭な倍音構造なし)**で一貫して弦楽器の特徴が出なかった。ここで初めて「Tracklib動画のオーディオトラック自体が、カード表示と同期した単離サンプル音声ではなく、プロモーション用の汎用BGM/編集音声である可能性」を疑い、音源をTracklib系列から完全に切り替えた。YouTube「Kitty with the Bent Frame (Sample in Loop)」(単離ループ専門chの投稿、gMaS_tjiv0I、`yt-dlp`で取得)から抽出したところ、明瞭な倍音構造+周期的なスウェル(オーケストラ弦楽器のアタック〜ディケイ)が確認でき、それまでの音とは明確に別物だった。ピッチはドラムの教訓を踏まえ変更せずノーマライズのみ。
    - **教訓**: 同一系列の動画内で複数タイムスタンプを試して毎回失敗する場合、「切り抜き位置が悪い」ではなく「その動画のオーディオトラック自体が目的の音を含んでいない」を疑うべき局面がある(4回同じ動画内で微調整しても改善しなかった時点がその閾値)。`yt-dlp`はこの環境に導入済みで、単離サンプル/ループ専門の投稿がYouTubeにしばしば存在するため、一次映像(Tracklib等の二次編集コンテンツ)で行き詰まったら早めに切り替え候補として検討する。
    - **検証**: `npm run check` 0 error(HTML/CSS無変更、アセットのみ差し替え)。スペクトログラム比較で新Kitty音源の倍音構造を確認、レンダー実物で試聴用クリップを送付済み。
