...
- **時間差表示（手動固定）**: ✂ボタンのモーダルに「⏱ 間で魅せる」タブ。開くとその行の「現在有効な」カット位置（手動固定済みならそれ、無ければ自動判定＝0.35s以上の間をそのまま再現）が選択済みで出る。位置を編集して「この位置で確定」→ `cue.stagger` に保存（空で確定＝この行は強制で一括表示）。「自動に戻す」で `cue.stagger` を消して閾値ベースの自動判定に戻す。自然な間が無い位置でも強制できる。✂ボタンは手動固定済みの行だけ色が変わる。
...
- **間の無い密なラップ行のflow reveal（2026-07-25追加）**: ポーズが検出できない行（rapが途切れず流れる箇所）は上記の間ベース分割が発動せず、行全体が最大数秒間1ブロックで静止して見える（実測タイミング自体は合っていても「ズレて見える」原因になる）。実発声span 2.0秒以上・6語以上の行を対象に、一定語数（既定3語・`--flow-group`）ごとに実発声タイミングで区切って流す。`--flow-min-words` / `--flow-min-span` / `--no-flow`で調整可能。lose-yourselfでは該当68行のうちspan2.0s以上の行が対象になり、segsを持つキューが9→47に増えた。
- **訳の表示タイミングとsegsの整合**: segsを持つ行は、訳(jp)は必ず最後の語群が表示されてから出す（アンチスポイラー最優先）。`--jp-max-delay`はsegsが無い行にのみ効く。`--jp-min-show`はどうしても間に合わない時だけ訳の表示を前倒しする最終手段のフロア（既定0.3秒）。
